"""
SyncNet GCN Baseline Training Engine
====================================
Implements a 2-layer Graph Convolutional Network (GCNConv) model using PyTorch Geometric
and trains on GPU (cuda:0).

Outputs:
- Test metrics (Accuracy, F1, ROC-AUC)
- Checkpoint: models/gcn_baseline.pt
"""
...
def train_gcn():
    ...
    print("=" * 60)
    print("SYNCNET — GCN BASELINE MODEL TRAINING (PyG GPU)")
    print("=" * 60)
    ...
    model = GCNNet(in_channels=in_channels, hidden_channels=32, out_channels=2, dropout=0.3).to(device)
    ...
    ckpt_path = os.path.join(MODELS_DIR, "gcn_baseline.pt")
    torch.save(model.state_dict(), ckpt_path)
    
    print(f"\n[x] SUCCESS: Saved GCN model weights to {ckpt_path}")
    print("=" * 60)
    
    return {
        "Name": "PyG GCN Baseline",
        "Accuracy": acc,
        "Precision": prec,
        "Recall": rec,
        "F1": f1,
